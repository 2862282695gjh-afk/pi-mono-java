...
覆盖合法及空名称、64/65 字符边界、首尾和连续连字符、首次发布失败、
...
历史版本中的“目录回退名称”只记录旧实现测试，不是允许缺少 name 的决策。
已合入设计 fd86049 明确要求原始非空字符串、与目录逐字符一致，不回退、不强制转换。
该缺口的实现与跨消费者验收补充见 [ADR-0056](0056-enforce-declared-skill-name.html)；
本 ADR 的共享严格规则和常量归属仍有效。
...
| 版本 | 日期 | 说明 |
|---|---|---|
| 1.3.1 | 2026-09-07 | 纠正目录回退测试说明，关联原始名称与目录身份补齐记录；不改变既有严格名称决策。 |
