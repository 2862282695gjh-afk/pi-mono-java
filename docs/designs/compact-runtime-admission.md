...
版本：1.0.0 · 日期：2026-09-07 · 串行切片 6b2a。

## 1. Context 与交付边界

[6b1](compact-runtime-lifecycle.md) 仅执行已经准入的压缩。完整 6b2 的格式化 WIP 达到模块新增
858 行，尚需补接入链路验证，超过 850 行预算。按已确认的相邻职责边界拆为两个串行 PR，
本切片只交付数据库端口；Runtime 接入及资源回收由 6b2b 在本 PR 合入后另建分支完成。
...
- 模块新增 531 行，镜像后非文档新增 1062/2000 行，低于 1800 软上限。
