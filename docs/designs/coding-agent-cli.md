...
4. **生产工具入口统一到 `ToolCatalog`**：Spring 工具、`ExtensionRegistry` 工具、声明式进程工具与 MCP 工具先汇聚为 catalog 快照，再由 `ToolSelection` 统一处理 `--tools` / `--no-tools` / settings 中的 `tools.enabled` / `tools.include` / `tools.exclude` / `tools.noTools`，并在 catalog refresh 时应用 `allowProjectTools` / `allowUserTools` / `allowToolReplacement` / `mcpEnabled` 等来源开关；`CampusClawCommand` 不再直接过滤 `List<AgentTool>`；cron 通过 `agent-core` 的 `ToolProvider` adapter 复用同一 catalog；
