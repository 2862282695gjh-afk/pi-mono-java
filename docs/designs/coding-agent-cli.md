...
本版本同时记录国际化资源包的目标设计：英文和简体中文资源将显式放入 `i18n/` 目录，且不保留基础 `messages.properties`。该调整尚未在分析基线实现，属于目标态架构变更。
...
| 当前错误国际化依赖根目录资源包 | `modules/coding-agent-cli/src/main/resources/messages.properties`、`messages_zh_CN.properties` |
| 当前语言选择手工检查请求头前缀 | `RuntimeRequestContext#locale`、`RuntimeRequestContext#language` |
| 当前 HTTP 与 SSE 错误通过 MessageSource 取文案 | `RuntimeExceptionHandler#response`、`RuntimeTerminalEventFactory#emitError` |
...
分析基线中的英文资源位于根目录 `messages.properties`，中文资源位于根目录
...
[ADR-0016](../decisions/0016-explicit-locale-message-bundles.html)。
...
| 版本 | 日期 | 说明 |
|---|---|---|
| 2.4.0 | 2026-08-20 | 明确无基础资源包的双 Locale 目录、显式 MessageSource、标准语言协商和错误消息国际化边界 |
