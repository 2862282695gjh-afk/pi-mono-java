...
待开发者补充。代码仓库未提供独立的 README/`*-design.md`；从仓库根 `CLAUDE.md` 与 `docs/module-architecture.md` 可知 agent-core 是 CampusClaw 项目（前身 pi-mono-java）的 "agent runtime" 抽象层，承载 LLM 与工具执行循环。可推断为"演进式架构治理"——把分散在 CLI / TUI 中的 agent 编排逻辑下沉到独立 lib，便于 cron / coding-agent-cli 多上游复用。
...
- **下游（grep 反查 import）**：`campusclaw-cron`、`campusclaw-coding-agent`
