...
|---|---|
| 文档版本 | 0.4.0 |
| 状态 | Implemented（过渡集成）；产品界面与 HTTP 1.37.0 已落地，生产公共 bridge 仍待设计 |
...
| HTTP 设计风格基线 | `chat-http-v1-design.md` 与 `contract/operations/01..11` 1.37.0；设计仓 `a012462527c1c25f912cb94113f28ed7d96d2761` |
...
| pi-mono-java 分析源码基线 | `7b655487df80685187ed0193f6578060615f7d43` |
...
| 源码证据 | 已实现行为 |
|---|---|
| `frontend/src/App.vue:13-160`，`createSession`、`resumeSession`、`submit` | 以 Agent 和会话为产品入口；会话列表当前仅保存在内存；运行中按当前 `steer/queue` 模式提交，成功前不清空输入。 |
| `frontend/src/App.vue:163-276` | 产品主界面只呈现侧栏、Agent/会话标题、模型、深度思考、执行状态、停止、对话和 Composer；开发者诊断仅在 `import.meta.env.DEV` 下出现。 |
| `frontend/src/composables/useRuntimeApi.ts:143-182` | `POST /events` 请求体只发送 `message/file_ids`；响应头接受后在后台消费请求级 SSE；Steer、FollowUp、Abort 对齐 1.37.0。 |
| `frontend/src/composables/useRuntimeApi.ts:235-319` | UTF-8 增量解析 SSE，持久化事件覆盖瞬时预览；流结束后从第一页读取历史；不自动重放任何副作用 POST。 |
| `frontend/src/projectors/runtimeEventProjector.ts:9-130` | User、Assistant、Thinking 与工具生命周期投影为稳定对话 turn 和活动卡，不在产品 UI 展示原始事件名或 payload。 |
...
- 对齐 HTTP 1.37.0，同时明确生产前端仍缺少的公共契约。
...
[PlantUML 源码：`runtime_event_ui_projection`](campusclaw-frontend/diagram.puml#L69)
...
- [ADR-0018：产品前端隔离 Runtime 调试协议](../decisions/0018-campusclaw-product-frontend-boundary.md)
  （Accepted）：生产 UI 使用公共 bridge；当前直接 Runtime adapter 只作为过渡集成。

## 13. 实施分期

1. **已完成：产品前端壳**：导航、三类状态、对话、活动卡、Composer、响应式和安全错误文案。
2. **已完成：过渡 Runtime adapter**：对齐 HTTP 1.37.0、请求级 SSE、历史去重、ETag 和运行控制；不接收浏览器凭据。
...
5. **下一步：测试固化**：把本轮契约桩与多视口手工验收升级为可重复的单元/E2E/视觉回归测试。
...
- 安全测试：当前过渡构建的产品 DOM 和错误 UI 不出现内部凭据、`session_id`、ETag 或原始 SSE；
