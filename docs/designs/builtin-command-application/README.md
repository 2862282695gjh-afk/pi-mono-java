...
版本：1.0.1 · 日期：2026-09-07 · 状态：内部 Builtin 应用接入，公共 HTTP 未发布。
...
2026-09-07 用户进一步确认：Skill 执行沿用普通消息的 SSE 响应；Builtin 仍返回普通 JSON。
该决定补充下列只读设计基线，不表示本 PR 已实现 Skill 或发布共享 HTTP。
后续 Skill 在显式选择与绑定检查后复用普通消息执行、SSE、持久化和恢复；额外的 Skill 版本/正文私有快照
按 Skill 设计 §4 不属于当前实现要求，不作为后续开发前置，也不另增历史类型或保留制度。
设计仓只读，本实现不替用户新增产品规则。
