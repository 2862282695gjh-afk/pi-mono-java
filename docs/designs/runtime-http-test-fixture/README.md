...
| 源码路径与符号 | 基线观察行为 |
|---|---|
| `modules/coding-agent-cli/src/test/java/com/campusclaw/codingagent/runtimeapi/web/RuntimeHttpProcessOpenGaussIT.java`：`prepareRuntimeFiles/startRuntime/ModelStub` | 一个 694 行测试类内私有地管理目录、JAR 进程、HTTP 和模型桩；旧夹具只写 settings/SYSTEM，使用本地 customModels 与 `/v1/chat/completions`。 |
...
| `modules/coding-agent-cli/src/main/java/com/campusclaw/codingagent/runtimeapi/model/MateRuntimeModelManager.java`：`resolveModel` | 从绑定模型 ID 创建 Mate Provider 模型，不消费旧夹具的本地 customModels。 |
...
旧夹具与主干生产配置已经漂移；因此本次不仅移动方法，还恢复当前主干实际入口的验收。
这是 **Java 测试架构调整**，不以 pi 作为夹具设计基线，不把 Java 跨进程设施描述成上游行为。
...
- `RuntimeHttpProcessFixture`：同包可复用的测试设施，负责完整目录、启动环境、真实 HTTP 帮助方法及资源。
...
- `RuntimeHttpProcessOpenGaussIT`：保留领域断言的消费者；既有 Events 场景和新增重启场景共用夹具。
...
4. 使用者必须指向预先初始化的专用测试数据库；夹具不初始化表，也不对用户数据库运行安装 DDL。
5. 模型响应计数用 AtomicInteger；夹具拥有虚拟线程 executor。关闭先停止服务，再中断并关闭 executor，
...
- 5 个夹具单元：完整真实缓存与重新读取、损坏缓存拒绝、真实模型 HTTP/gate、活动 gate 关闭和非 Chat 路径拒绝。
