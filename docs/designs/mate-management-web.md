...
| 已评审实现 | `65fb8eabbaa1a7dd63c2777280e19c8bdf5be8eb` |
...
本次采用追加方式：保留已有清单，增加关闭管理端口的属性以及两个管理上下文排除项。通用 `modules/coding-agent-cli` 不直接依赖公司父 POM，配置保持不变。
...
- **自动配置边界：** 保留原有排除清单，并追加 `ManagementContextAutoConfiguration` 与 `ServletManagementContextAutoConfiguration`；已有的 `WebEndpointAutoConfiguration` 不重复声明。

## 3. 架构与配置流

启动时，Spring Boot 先加载 `mate-campusclaw/src/main/resources/application.properties`，再由公司父项目传入的 Actuator 自动配置读取管理属性。目标配置同时作用于管理端口、Endpoint 默认开关和自动配置导入边界。业务 Servlet Web 服务仍使用 `server.*` 配置启动。

本次没有新增运行时组件、调用链或状态转换，三组静态配置之间不存在需要图示才能澄清的拓扑，因此不新增 PlantUML 图。

## 4. 设计决策

正式决策见 [ADR-0038：追加关闭 Mate 管理 Web 面](../decisions/0038-disable-mate-management-web.html)。

### 4.1 保留原排除项并追加管理上下文保护

不把现有 30 项清单替换为 3 项。替换会恢复大量与本次目标无关的 Actuator 自动配置，无法在缺少公司父 POM 的本地环境中证明兼容性。追加方案只改变缺失的管理端口和管理上下文边界。
...
新增测试直接加载真实 `application.properties`，断言管理端口、Endpoint 默认开关和 3 个关键管理 Web 自动配置排除项。测试文件加入同步排除清单，使后续模块镜像同步不会删除它。

## 5. 边界情况

- 本次不删除或放宽任何现有 Actuator 排除项。
...
- **可靠性：** 追加改动避免意外恢复原有被排除的 Actuator 组件。
- **安全性：** 显式关闭管理端口和管理 Web 上下文，避免公司父 POM 传入未预期的管理 HTTP 面。
- **可维护性：** 保留现有清单的兼容边界；测试明确锁定本次新增的关键项。
...
- `spring.autoconfigure.exclude` 在原列表末尾新增两个类名。

## 8. 实现证据

以下内容来自已评审实现 `65fb8eabbaa1a7dd63c2777280e19c8bdf5be8eb`：

| 目标行为 | 实现证据 |
|---|---|
| 关闭管理端口并保留 Endpoint 默认禁用 | `mate-campusclaw/src/main/resources/application.properties:1-3` |
| 保留原清单并追加两个 Management Context 排除 | `mate-campusclaw/src/main/resources/application.properties:4-36` |
| 业务服务端口保持原配置 | `mate-campusclaw/src/main/resources/application.properties:40-41` |
| 真实配置加载测试覆盖新增边界 | `mate-campusclaw/src/test/java/com/huawei/hicampus/mate/matecampusclaw/codingagent/config/ManagementConfigurationTest.java:19-39` |
| 公司专用测试受镜像同步保护 | `scripts/sync-mate-exclude.txt:9-10` |

## 9. 测试与验证

- 执行 `ManagementConfigurationTest`，验证真实配置解析结果。
