...
| DEF-007 | coding-agent-cli | `HttpMateToolClient` 四个 Mate RPC stub 填充真实 HTTP 调用：`queryToolIdsByAgentId` / `queryToolIdsBySkillId` / `queryToolMetaByIds` / `invokeToolById`。签名与调用点已定（见 `feat/mate-tool-client` 分支），仅需填方法体。四个方法均接收 `MateCredentials`（AppKey: X-HW-ID+X-HW-APPKEY; JWT: X-HW-ID+Authorization Bearer）。 | Mate 工具服务内部接口确定后 | — |
