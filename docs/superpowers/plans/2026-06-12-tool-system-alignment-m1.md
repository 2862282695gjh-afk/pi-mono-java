...
- [x] **Step 1: Write failing tests** for `prepareArguments`, `BeforeToolCallResult.argsOverride`, and `defaultExecutionMode` forcing sequential execution.
- [x] **Step 2: Run** `./mvnw -pl modules/agent-core test -Dtest=ToolExecutionPipelineTest` and verify the new tests fail because the APIs/behavior do not exist.
- [x] **Step 3: Add default methods** to `AgentTool`: `prepareArguments(Map<String,Object>)` returning the raw args and `defaultExecutionMode()` returning `ToolExecutionMode.PARALLEL`.
- [x] **Step 4: Extend** `BeforeToolCallResult` with an immutable `argsOverride` map plus static helpers preserving `allow()` and `block(String)`.
- [x] **Step 5: Update pipeline** so argument preparation runs before schema validation, before-hook overrides are used for execution/events/after-hook, and `executeAll(..., PARALLEL, ...)` switches to sequential when any call's tool asks for `SEQUENTIAL`.
- [x] **Step 6: Re-run** `./mvnw -pl modules/agent-core test -Dtest=ToolExecutionPipelineTest`.
...
- [x] **Step 1: Write failing tests** covering Spring source passthrough, extension source passthrough, priority ordering, duplicate `ADD` diagnostics, `REPLACE`, `DISABLE`, `--tools`, excludes, and `noTools`.
- [x] **Step 2: Run** `./mvnw -pl modules/coding-agent-cli test -Dtest=ToolCatalogTest,ToolSelectionTest` and verify failures are missing classes.
- [x] **Step 3: Implement records/enums/interfaces** with immutable lists/maps and diagnostic strings.
- [x] **Step 4: Implement `DefaultToolCatalog`** with copy-on-write `refresh()`, `snapshot()`, and `resolve(ToolSelection)`.
- [x] **Step 5: Implement Spring and extension sources** as Spring components where dependencies are available, with nullable/empty extension registry support.
- [x] **Step 6: Re-run** the focused catalog tests.
...
- [x] **Step 1: Write failing CLI tests** proving `resolveEffectiveTools()` delegates to `ToolCatalog.resolve(ToolSelection)` for default, include-list, and no-tools cases.
- [x] **Step 2: Run** `./mvnw -pl modules/coding-agent-cli test -Dtest=CampusClawCommandTest`.
- [x] **Step 3: Inject `ToolCatalog`** into `CampusClawCommand`, keep existing constructor compatibility by creating a catalog from `List<AgentTool>` when Spring has not supplied one.
- [x] **Step 4: Replace direct list filtering** with `ToolSelection.fromCli(toolsFilter, noTools)` and `toolCatalog.resolve(...)`.
- [x] **Step 5: Re-run** the focused CLI tests.
...
- [x] **Step 1: Run formatting** with `./mvnw spotless:apply`.
- [x] **Step 2: Run agent-core tests** with `./mvnw -pl modules/agent-core test`.
- [x] **Step 3: Run coding-agent-cli tests** with `./mvnw -pl modules/coding-agent-cli test`.
- [x] **Step 4: Inspect diff** with `git diff --stat` and `git diff --check`.
