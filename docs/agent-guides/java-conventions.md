# Java conventions and build checks

下面分别注明构建强制规则和评审约定。构建规则违规会让 `./mvnw validate`（Checkstyle）或 `./mvnw spotless:check`（palantirJavaFormat）失败。配置了 `scripts/claude-hooks/checkstyle-on-stop.sh` 的环境可能自动执行检查；以本次实际输出为准，避免重复已完成且仍有效的检查。
...
未完成或暂缓实现的工作改记录到 [`docs/DEFERRED.md`](../DEFERRED.md)；如果是已上线产品的缺陷，请直接在 issue tracker 立单。代码里残留 TODO/FIXME 会让客户误解为已知缺陷。
...
**不**由 Checkstyle 拦截——AST 级判断「`new Thread(...)` 后是否在 `start()` 之前调用了 `setUncaughtExceptionHandler`」复杂度与收益不成正比，靠规范+评审保证。修改涉及的线程时遵守本规则；无关存量问题单独记录，不自动扩大本次改动。
...
### 写完 Java 之后
如当前环境未运行对应检查，对受影响的 Java 模块执行格式和 Checkstyle 检查。修复命令：
