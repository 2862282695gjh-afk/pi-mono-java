...
基于 pi-mono TypeScript 版本，使用 Java 21 + Spring Boot 3.x 实现 4 个核心模块：pi-ai、pi-agent-core、pi-coding-agent、pi-tui，以支撑 openclaw 项目。
...
```
pi-coding-agent (Spring Boot 应用)
    ├── pi-agent-core (Spring Boot 库)
    │     └── pi-ai (Spring Boot 库)
    └── pi-tui (纯 Java 库，不依赖 Spring)
```
...
| 模块 | Gradle 路径 | 包名 | 角色 |
|------|------------|------|------|
| **pi-ai** | `:modules:pi-ai` | `com.mariozechner.pi.ai` | 统一多 LLM 提供商 API、类型定义、流式协议 |
| **pi-agent-core** | `:modules:pi-agent-core` | `com.mariozechner.pi.agent` | 代理运行时、状态管理、工具框架、Agent 循环 |
| **pi-coding-agent** | `:modules:pi-coding-agent` | `com.mariozechner.pi.codingagent` | CLI 入口、内置工具、系统提示、运行模式 |
| **pi-tui** | `:modules:pi-tui` | `com.mariozechner.pi.tui` | 终端 UI 组件、ANSI 渲染、差量更新 |
...
7. **模块边界**: pi-ai/pi-agent-core 作为 Spring Boot starter 库，pi-coding-agent 作为 Spring Boot 应用，pi-tui 不依赖 Spring
...
├── pi-mono-java-review.md        # 本文档
├── modules/
│   ├── ai/                       # pi-ai 模块
│   │   ├── build.gradle.kts
│   │   └── src/main/java/com/mariozechner/pi/ai/
│   ├── agent-core/               # pi-agent-core 模块
│   │   ├── build.gradle.kts
│   │   └── src/main/java/com/mariozechner/pi/agent/
│   ├── coding-agent-cli/         # pi-coding-agent 模块
│   │   ├── build.gradle.kts
