...
DROP TABLE IF EXISTS t_tmp_session_new_events;
DROP TABLE IF EXISTS t_tmp_session_event_migration_sessions;
DROP TABLE IF EXISTS t_tmp_session_event_migration_issues;
DROP TABLE IF EXISTS t_tmp_session_all_public_events;
DROP TABLE IF EXISTS t_tmp_session_event_candidates;
DROP TABLE IF EXISTS t_tmp_session_event_decisions;

CREATE TEMP TABLE t_tmp_session_event_decisions (
...
INSERT INTO t_tmp_session_event_decisions (session_id, anchor_entry_id, event_count, decision_source)
...
CREATE TEMP TABLE t_tmp_session_event_candidates (
    session_id      VARCHAR(128)   NOT NULL,
    anchor_entry_id VARCHAR(128)   NOT NULL,
...
INSERT INTO t_tmp_session_event_candidates
...
FROM t_session_entries entry
JOIN t_tmp_session_event_decisions decision
...
CREATE TEMP TABLE t_tmp_session_all_public_events (
    session_id      VARCHAR(128) NOT NULL,
    event_id        VARCHAR(128) NOT NULL,
    anchor_entry_id VARCHAR(128) NOT NULL,
    type            VARCHAR(64)  NOT NULL,
...
INSERT INTO t_tmp_session_all_public_events
SELECT session_id, event_id, anchor_entry_id, type, payload
FROM t_session_events;

INSERT INTO t_tmp_session_all_public_events
SELECT session_id, event_id, anchor_entry_id, type, payload
FROM t_tmp_session_event_candidates;

CREATE TEMP TABLE t_tmp_session_event_migration_issues (
    session_id VARCHAR(128) NOT NULL,
...
FROM t_session_entries entry
LEFT JOIN t_tmp_session_event_decisions decision
  ON decision.session_id = entry.session_id
 AND decision.anchor_entry_id = entry.id
...
FROM t_tmp_session_all_public_events
...
AND NOT EXISTS (
    SELECT 1
    FROM t_tmp_session_all_public_events source
...
AND NOT EXISTS (
    SELECT 1
    FROM t_tmp_session_all_public_events target
...
AND NOT EXISTS (
    SELECT 1
    FROM t_tmp_session_all_public_events tool_call
...
CREATE TEMP TABLE t_tmp_session_event_migration_sessions (
    session_id VARCHAR(128) PRIMARY KEY
);

INSERT INTO t_tmp_session_event_migration_sessions
SELECT session.id
...
WHERE NOT EXISTS (
    SELECT 1
    FROM t_tmp_session_event_migration_issues issue
...
CREATE TEMP TABLE t_tmp_session_new_events (
    session_id      VARCHAR(128)   NOT NULL,
    event_id        VARCHAR(128)   NOT NULL,
...
INSERT INTO t_tmp_session_new_events
...
FROM t_tmp_session_event_candidates candidate
JOIN t_tmp_session_event_migration_sessions migration
...
USING t_tmp_session_event_migration_sessions migration
...
DROP TABLE t_tmp_session_new_events;
DROP TABLE t_tmp_session_event_migration_sessions;
DROP TABLE t_tmp_session_event_migration_issues;
DROP TABLE t_tmp_session_all_public_events;
DROP TABLE t_tmp_session_event_candidates;
DROP TABLE t_tmp_session_event_decisions;
