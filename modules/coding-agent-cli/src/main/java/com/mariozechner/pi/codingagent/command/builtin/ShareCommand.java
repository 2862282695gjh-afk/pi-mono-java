...
import com.mariozechner.pi.codingagent.export.HtmlExporter;
...
public class ShareCommand implements SlashCommand {
    ...
    public void execute(SlashCommandContext context, String arguments) {
        ...
        String html = HtmlExporter.export(messages, "Pi Session", modelId);

        try {
            Path tmpFile = Files.createTempFile("pi-share-", ".html");
            Files.writeString(tmpFile, html);

            // Use gh CLI to create a gist
            var process = new ProcessBuilder(
                    "gh", "gist", "create",
                    "--desc", "Pi session (" + messages.size() + " messages)",
                    tmpFile.toString()
