...
| `CAMPUS_OPS_RULES_PATH` | `device-inspection-re/rules/rules_re.json` |
...
- **生产（MySQL 8.0+）**：`templates/schema.mysql.sql` + `scripts/db_backend.py`

| 变量 | 说明 |
|------|------|
| `DEVICE_INSPECTION_RE_DB_BACKEND` | `sqlite`（默认）或 `mysql` |
| `DEVICE_INSPECTION_RE_DB_URL` | `mysql://user:pass@host:3306/campus_inspection` |
| `DEVICE_INSPECTION_RE_DB_HOST/PORT/USER/PASSWORD/NAME` | 分项配置（无 URL 时） |
| `CAMPUS_OPS_DB_BACKEND` / `CAMPUS_OPS_DB_URL` | campus 侧别名 |

| 表 | 说明 |
|----|------|
| `inspection_runs` | 每次巡检一条（run_id、end_ts、告警数量、scope_device_types） |
| `inspection_alarms` | 每条告警一行（设备、规则、原因、建议、楼栋等） |

**写入**：`device-inspection-re/scripts/judge_rules_re.py`（或 `POST /api/v1/inspection/run`）  
**读取**：`GET /api/v1/alarms`、`/alarms/stats` 等（默认 `runId=latest`）

### 启动 REST API（对外集成）

**生产 / 对外**（监听 `0.0.0.0`，带 CORS）：

```bash
# MySQL
export DEVICE_INSPECTION_RE_DB_BACKEND=mysql
export DEVICE_INSPECTION_RE_DB_URL=mysql://campus:campus@127.0.0.1:3306/campus_inspection
python <skill-path>/api_server.py
```

**本地 mock**（仅 127.0.0.1）：

```bash
python <skill-path>/mock_api_server.py
```

OpenAPI 契约：`docs/openapi/device-inspection-api.yaml`

MySQL 本地容器：

```bash
docker compose -f <skill-path>/docker-compose.mysql.yml up -d
```
