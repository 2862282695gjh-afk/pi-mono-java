"""
Persistence for device-inspection-re (rules_re.json).

Backends: SQLite (default mock) or MySQL 8.0+ (production).
See scripts/db_backend.py and templates/schema.mysql.sql.
...
from db_backend import DbConnection, connect, db_path, init_schema, table_columns


def _connect(*, require_exists: bool = False) -> DbConnection:
    return connect(require_exists=require_exists, init=True)
...
def load_inspection_doc(*, run_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        if "not found" in str(exc):
            raise
        raise FileNotFoundError("no inspection in database; run judge_rules_re.py first") from exc
