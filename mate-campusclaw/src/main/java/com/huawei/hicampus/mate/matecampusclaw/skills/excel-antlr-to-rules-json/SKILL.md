...
| 清理中间文件 | `python <skill-path>/scripts/cleanup_rules_intermediates.py` |
...
**路径说明**：`<skill-path>` = 本 skill 根目录；规则文件统一放在 `<skills-dir>/device-inspection-re/rules/`。
...
4. 不确定时每轮最多 1 个问题  
5. 全部成功 → `verify_rules_re.py` → 用户确认后写 `device-inspection-re/rules/rules_re.json` → **`cleanup_rules_intermediates.py` 删除中间产物**  
...
| 4 | 用户确认 | `device-inspection-re/rules/rules_re.json` | 候选 → 正式 |
...
**路径说明**：`<skill-path>` = excel-antlr 本 skill；`<inspection-path>` = 同级 `device-inspection-re` skill 根目录。
...
python <skill-path>/scripts/excel_to_rules.py \
  path/to/故障规则_patched.xlsx \
  <inspection-path>/rules/_candidate_rules_re.json \
  --report <inspection-path>/rules/compile_report.json

python <skill-path>/scripts/verify_rules_re.py \
  <inspection-path>/rules/_candidate_rules_re.json

# 确认无误后：候选 → 正式，再清理中间文件
cp <inspection-path>/rules/_candidate_rules_re.json <inspection-path>/rules/rules_re.json
python <skill-path>/scripts/cleanup_rules_intermediates.py
```
...
# 确认无误后
copy <inspection-path>\rules\_candidate_rules_re.json <inspection-path>\rules\rules_re.json
python <skill-path>\scripts\cleanup_rules_intermediates.py
```
...
```bash
python <skill-path>/scripts/generate_mock_fixtures.py \
  <inspection-path>/rules/rules_re.json \
...
```bash
python <skill-path>/scripts/apply_excel_patch.py \
  <inspection-path>/rules/patches.json \
  path/to/故障规则.xlsx
```
