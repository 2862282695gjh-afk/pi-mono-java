...
| 单元格 | JSON `effective.metric` | 含义 |
|--------|-------------------------|------|
| `无需设置` | `last_point` | 仅判定**最近一个采样点**是否满足触发式（瞬时告警） |
| `30min内，90%` | `ratio_true` | 1800s 滚动窗口内，满足占比 ≥ 0.9 |
| `15min内，90%` | `ratio_true` | 900s 滚动窗口内，满足占比 ≥ 0.9 |
...
| 删除 | 位置 | 保留到何时 |
|------|------|------------|
| `_candidate_rules_re.json`、`compile_report*.json` | `device-inspection-re/rules/` | 已复制为 `rules_re.json` 且验证通过 |
| `patches.json` | `device-inspection-re/rules/` | `apply_excel_patch` 已生成 patched xlsx 且编译成功后 |
