...
- **规则文件缺失**（`rules_not_found` / 未找到 `rules_re.json`）：向用户说明，引导 **excel-antlr-to-rules-json** 或放置 `device-inspection-re/rules/rules_re.json`，**立即结束 campus-device-ops / 巡检相关 skill**，勿委托 `run_inspection` 后继续推送或工单。
