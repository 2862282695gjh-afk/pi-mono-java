"""
Read inspection results from device-inspection-re shared database.

Backends: SQLite (default mock) or MySQL 8.0+ (production).
See scripts/db_backend.py and device-inspection-re/templates/schema.mysql.sql.
...
from db_backend import DbConnection, connect, table_columns
...
def _connect() -> DbConnection:
    return connect(require_exists=True, init=False)
...
def list_inspection_runs(*, limit: int = 20) -> List[Dict[str, Any]]:
    with _connect() as conn:
        cols = table_columns(conn, "inspection_runs")
...
def query_alarms(
    *,
    ...
    with _connect() as conn:
        cols = table_columns(conn, "inspection_alarms")
        optional = [
            "device_name",
            "building",
            "device_type",
    ...
    for r in rows:
        ...
        item = {
            ...
            "building": (r.get("building") or "") or meta.get("building", ""),
            "deviceName": (r.get("device_name") or "") or meta.get("name", ""),
            "deviceType": (r.get("device_type") or "") or meta.get("deviceType", ""),
            "component": (r.get("component") or "") or meta.get("component", ""),
            "assigneeId": aid,
            "assigneeName": aname,
        }
        if "floor" in r:
            item["floor"] = r.get("floor") or meta.get("floor", "")
        elif meta.get("floor"):
            item["floor"] = meta.get("floor", "")
        if "room" in r:
            item["room"] = r.get("room") or meta.get("room", "")
        elif meta.get("room"):
...
def load_inspection_doc(*, run_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    if "rules_kind" in run:
        doc["rulesKind"] = run["rules_kind"]
    if "scope_device_types" in run:
        doc["deviceTypeFilter"] = _decode_scope_device_types(run.get("scope_device_types"))
    return doc
