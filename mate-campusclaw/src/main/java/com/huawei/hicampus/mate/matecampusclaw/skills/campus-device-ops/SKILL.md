...
**路径说明**：`<skill-path>` 为本 skill 根目录（含 `SKILL.md` 的目录）；`<skills-dir>` 为其父目录（与 `device-inspection-re` 等并列）。脚本内部按 `__file__` 解析路径，**不依赖**固定仓库根或 `.campusclaw` 位置。
...
pip install -r <skills-dir>/device-inspection-re/requirements.txt

# 1. 设备巡检（写库）
python <skills-dir>/device-inspection-re/scripts/judge_rules_re.py --json

# 2. 园区运维（读库）
python <skill-path>/scripts/query_alarms.py --json
python <skill-path>/scripts/alarm_stats.py --json
python <skill-path>/scripts/build_qa_context.py --json
python <skill-path>/scripts/push_alert_digest.py --write-ai-message --json
...
# 一键验收（先巡检 → 统计 → 推送 → 问答）
python <skill-path>/scripts/verify_openclaw_pipeline.py --skip-inspection
```

也可在 skill 根目录下直接运行（`<skill-path>` 为当前目录）：

```bash
cd <skill-path>
pip install -r requirements.txt
python scripts/query_alarms.py --json
```

OpenClaw / 自定义 workspace 示例（`<workspace>` 为包含 `skills/` 的目录）：

```bash
python <workspace>/skills/campus-device-ops/scripts/openclaw_env.py  # 可选：打印 pinned env
python <workspace>/skills/device-inspection-re/scripts/judge_rules_re.py --json
python <workspace>/skills/campus-device-ops/scripts/query_alarms.py --building 能源中心 --json
python <workspace>/skills/campus-device-ops/scripts/push_alert_digest.py --write-ai-message --json
python <workspace>/skills/campus-device-ops/scripts/verify_openclaw_pipeline.py --skip-inspection
```
