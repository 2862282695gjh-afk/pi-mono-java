...
DEFAULT_RULES_PATH = HERE / "rules" / "rules_re.json"
...
def _default_rules_path() -> Path:
    if DEFAULT_OPENCLAW_UNIFIED.exists():
        return DEFAULT_OPENCLAW_UNIFIED
...
def _symbol_names_in_expression(text: str) -> Set[str]:
    """从 rule_engine 文本中粗提取标识符，用于合并进 meta.points（补依赖）。"""
    out: Set[str] = set()
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    results = {"fault_devices": fault_devices, "alerts_by_device": alerts_by_device, "end_ts": end_ts}
    if args.json:
