"""
SQLite persistence for device-inspection (mock DB).
...
def db_path() -> Path:
    env = os.environ.get("DEVICE_INSPECTION_DB_PATH", "").strip()
    if env:
        return Path(env).expanduser().resolve()
    return skill_root() / "mock_fixtures" / "device_inspection.db"
...
def init_schema(conn: sqlite3.Connection) -> None:
    conn.executescript(
        ...
        CREATE TABLE IF NOT EXISTS inspection_runs (
            ...
            rules_kind TEXT NOT NULL DEFAULT 'rules.json',
...
def load_inspection_doc(*, run_id: Optional[str] = None) -> Dict[str, Any]:
    rid = resolve_run_id(run_id)
    if not rid:
        raise FileNotFoundError("no inspection in database; run judge_rules.py first")
