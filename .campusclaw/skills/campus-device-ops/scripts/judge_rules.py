...
DEFAULT_MOCK_API_URL = "http://127.0.0.1:18083/fetch"
...
def _default_fixtures_dir() -> Path:
    ...
    return _SKILL_ROOT / "mock_fixtures" / "timeseries"
...
def _try_start_mock_api_server() -> None:
    if _is_port_listening("127.0.0.1", DEFAULT_FETCH_PORT):
        return
    script = _SKILL_ROOT / "fetch_api_server.py"
...
def _call_mock_api(*, queries: List[Dict[str, Any]], end_ts: float) -> Dict[str, Any]:
    url = os.environ.get("CAMPUS_OPS_FETCH_API_URL", DEFAULT_MOCK_API_URL).strip() or DEFAULT_MOCK_API_URL
...
def main(argv: Optional[List[str]] = None) -> int:
    os.environ.setdefault("CAMPUS_OPS_TIMESERIES_FIXTURES_DIR", str(_default_fixtures_dir()))

    p = argparse.ArgumentParser(description="园区运维巡检（rules_re.json / 本 skill 独立）")
    ...
    if not args.no_save_db:
        print(
            "campus-device-ops judge_rules.py does not write DB; "
            "use device-inspection-re/scripts/judge_rules_re.py to persist.",
            file=sys.stderr,
        )
