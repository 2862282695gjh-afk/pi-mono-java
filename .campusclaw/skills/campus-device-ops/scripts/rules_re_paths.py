"""
rules_re.json path resolution — synced with device-inspection-re/scripts/rules_re_paths.py.
"""
...
RULES_NOT_FOUND_USER_HINT = (
    ...
    "或将其放到 workspace/rules/rules_re.json（OpenClaw）或 .campusclaw/rules/rules_re.json。\n"
...
def rules_re_candidate_paths() -> List[Path]:
    ...
    add(root / "rules" / "rules_re.json")
    add(Path.home() / ".openclaw" / "workspace" / "rules" / "rules_re.json")
    add(root / "rule-engines-pack" / "02-rule-engine-pypi" / "rules" / "rules_re.json")
    return paths
