...
6. **规则文件缺失**：若 `judge_rules_re.py` 返回 `rules_not_found` 或 exit code 非 0 且提示未找到 `rules_re.json`，**向用户说明原因并立即结束本 skill**，勿继续 `format_inspection_report` / `alarm_stats` / 工单等步骤。引导用户先用 **excel-antlr-to-rules-json** 编译规则，或放置 `workspace/rules/rules_re.json`。
